...
export async function GET() {
  ...
  const tasks = await prisma.task.findMany({ where, include: { assignedTo: true, reports: true }, orderBy: { createdAt: "desc" } });
...
export async function POST(req: NextRequest) {
  ...
  if (parsed.data.departmentId !== session.user.departmentId) return jsonError("Cannot create outside your department", 403);

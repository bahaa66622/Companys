...
export async function GET() {
  ...
  const users = await prisma.user.findMany({
    where: session.user.role === Role.SUPER_ADMIN ? {} : { companyId: session.user.companyId ?? undefined },
...
export async function PATCH(req: NextRequest) {
  ...
  const user = await prisma.user.update({
    ...
    data: { companyId: parsed.data.companyId, departmentId: parsed.data.departmentId },
